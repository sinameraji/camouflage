...
Surfaced while dogfooding `--ui camouflage` against a host application and looking at real screenshots through the new vhs harness.
...
| TP-2 | ✅ DONE — `1782753` | Splash height cap was 12 rows; clipped the host's 26-row pixel-art logo | Raised cap to `min(area.height/2, 30)`. Splash auto-dismisses on first user submit, so generosity is cheap. Later widened further in TP-4. | `crates/tui/src/draw.rs` |
...
| TP-4 | ✅ DONE — `da80961` | Splash responsive on small terminals | ANSI-aware `trim_empty_splash_lines` strips visually-empty leading/trailing rows from host-supplied splash (the host's logo had 5+6 wasted blank rows). Height cap widened from `area.height/2` to `area.height - 6`. Verified visually at 900×520. | `crates/tui/src/draw.rs` |
| TP-5 | ⬜ TODO | vhs Enter keystroke may not auto-dismiss splash | While running `host-splash.tape`, after `Type "hello" / Enter` the splash stayed pinned and `hello` stayed in the input box. Could be (a) vhs Enter not reaching the TUI through the pipeline, or (b) auto-dismiss-on-first-submit not firing under `--stdin-events`. Repro: `tests/visual/run.sh splash` — frame `03-after-submit.png`. | `crates/renderer/src/model.rs` auto-dismiss path, or vhs/PTY |
...
These came out of running `your-agent --ui camouflage --dangerously-allow-all -p "..."` end-to-end against a real host turn. They're real, reproducible, and block daily use.

| ID | Status | Bug | Reasoning / details | Where |
|----|--------|-----|---------------------|-------|
| TBR-1 | ✅ DONE — `dc861bc` | Tool execution rows show raw JSON-stringified arguments spanning many visual lines | `The host's call.function.arguments is a JSON string. Model put it directly into the row text. Fix: compact_command() helper truncates to 120 chars with ellipsis, collapses whitespace, strips control chars. Full payload retained on ToolState.command for X-overlay.` | `crates/renderer/src/model.rs` |
...
| TBR-5 | ⬜ TODO | Slash picker (`/`) does nothing when typed | `The host adapter does not yet emit SlashCommandsRegistered. The TUI picker overlay exists (Phase 2.6, commit 48fb2c7) but only triggers when commands are registered. Adapter fix: enumerate the host's slash commands and emit on session start.` | host adapter repo, `src/{ui-mode,emit-mode}.ts` |
...
| ID | Component | Status | Host consumers it unblocks | Reasoning |
|----|-----------|--------|----------------------------------|-----------|
| CC-1 | `SelectList` | ✅ DONE — `c7480b4 → aac4094` (camouflage) + `6556771` (host adapter) | session picker, checkpoint picker, theme picker, slash picker (generalised), resume picker, model picker | Highest leverage — single shape covers ~6 host React components. Shipped in 5 commits: protocol+payloads, model+Snapshot, TUI overlay+keys+outbound emit, Node SDK helper, host `resume` driver. Validated end-to-end with real session data. Existing hardcoded slash picker (Phase 2.6) is structurally a special-case SelectList — generalising it (deleting `SlashCommandsRegistered` in favor of `ShowSelectList`) is a follow-up cleanup. |
...
| CC-3 | `Toast` | 🗑️ REMOVED — `chore/remove-toast` | (was: "Saved" / "Authenticated" feedback) | Removed entirely on request: rolled back across protocol (`ShowToast` + payloads), renderer model/snapshot, TUI draw + Esc handling, headless validate/fixtures, Node SDK, fixtures, golden snapshot, and the `toasts.tape` visual test. Non-additive protocol change; lenient readers skip a stray `ShowToast`, strict `camouflage-validate` now rejects it. |
| CC-4 | `Wizard` | ✅ DONE — `03addfd` (composes Select/Confirm/Form steps; renderer intercepts sub-modal responses into wizard advance path) | onboarding flow, LSP wizard, command wizard | Multi-step; composes 1+2+3. |
| CC-5 | `Form` | ✅ DONE — `801a8fd` (text + password fields; Up/Down or Tab to navigate; boolean / select fields are follow-ups) | settings configuration, cloud token + endpoint configuration | Bigger; can wait for a real driver. |
...
Each component ships as a self-contained slice (one commit series): protocol event(s) → model state → renderer overlay → Snapshot projection field → Node SDK types update → host adapter wires it against an actual component being replaced. The host wiring is what proves the design.

### Host adapter follow-ups (on the host adapter repo, `camouflage-adapter` branch)

| ID | Status | Item | Reasoning |
|----|--------|------|-----------|
| ADP-1 | ⬜ TODO | Emit `SlashCommandsRegistered` listing the host's slash commands | Unblocks TBR-5. Source list in the host adapter's `src/commands/builtins.ts`. |
| ADP-6 | ⬜ TODO | Set `user_label` / `assistant_label` on `SessionStarted` (e.g. `"your-agent"`) | Unblocks the TP-3 branding for the real adapter. One-line change in `src/ui-mode.ts` where the first `cam.send("SessionStarted", …)` happens, plus `npm run build`. |
| ADP-7 | ⬜ TODO | Stale-dist guardrail — the host's `dist/index.js` shipped without the Splash-send code for an unknown period | Discovered 2026-05-24: the splash code lived in `src/` but the bundled `dist/` was older, so the logo never reached the renderer in practice. Either add a `prepack`/`prepublish` hook that fails when dist is older than src, or move to `tsx`/just-in-time so dist drift can't happen. |
| ADP-2 | ⬜ TODO | Wire `--ui camouflage` to use a SelectList for the resume picker (first CC-1 driver) | After CC-1 ships, replace the host's `src/ui/resume-picker.tsx` with a `ShowSelectList` emit + `SelectListResponse` handler. Validates the catalog design. |
| ADP-3 | ⬜ TODO | Host → npm-published `camouflage` package | Currently uses `file:../camouflage/sdk/node` for local development. Switch to a real npm version once we publish. |
| ADP-4 | ⏸ DEFERRED | Cost segment in `StatusUpdate` (mode/phase/elapsed/tokens/cost/branch) | Requires the host's cost-attribution machinery (complex; lives in `src/cost-attribution/`). Land after CC-1 → CC-3 demonstrate the catalog works. |
| ADP-5 | ⏸ DEFERRED | Mode cycling (`edit` / `plan` / `auto` + Shift+Tab) | Hardcoded to "edit" in `runUiMode`. Needs a host-protocol addition (`ModeChanged` event) and the host's mode controller. |
...
| ID | Status | Item | Reasoning |
|----|--------|------|-----------|
| ECO-1 | ⬜ TODO | Integration adapters bundle | Reference adapters for additional hosts (e.g., a Python adapter, Go adapter). |
...
| ECO-4 | ⬜ TODO | Migration guide (Ink → Camouflage) | Step-by-step "how to replace Ink" doc. Should reference the host adapter's Option-B migration as the worked example. |
...
| UX-10 | ⬜ TODO | Port the host's remaining 7 themes (we have 6 of 13) | Catppuccin Latte/Mocha, Everforest dark/light, Kanagawa, One Dark, Solarized dark/light. Data entry. |
...
**Node SDK (`camouflage` npm package) shipped + `--ui camouflage` mode in the host.** The user can now run `your-agent --ui camouflage -p "..."` and have Camouflage render directly in their terminal — single command, single process tree, no piping. This is the integration shape that paves the way to deleting Ink entirely (Option B).

Five-commit series on Camouflage main (`ebf748d` → `d826f89`) + camouflage-tui `--responses-fd` flag (`97af9dd`) + host `--ui camouflage` rewrite on the `camouflage-adapter` branch in the host adapter repo (`2f1c7de`).

**Ink-replacement-ready milestone shipped (tagged `v0.4.5`).** Three phases of focused work closed the gap between Camouflage and the host's React/Ink UI:

- **Phase 1** (5 slices on the host adapter repo's `camouflage-adapter` branch): adapter now emits `ToolExecutionStdout/Stderr`, supports `--multi-turn` mode, emits `StatusUpdate` (mode/phase/elapsed/tokens/branch), wires bidirectional `PermissionResponse`, emits `BackgroundTaskUpdate` for skills/memory/agent tasks.
...
The host can now realistically swap out Ink for Camouflage end-to-end. Remaining gaps are nice-to-haves (more themes, hierarchical help menu, queued-prompts row, etc.) — see "What's left".
...
| Workspace `fixtures/` | repo root | `host-mock.ndjson`, `adapter-simple.ndjson`, `all-event-types.ndjson` |
...
| F | `examples/host-mock` + integration verification | ✅ DONE — pty test confirmed all surfaces, `PermissionResponse{choice:"allow_once",request_id:"perm-1"}` round-trip |
...
| 2026-05-16 | Host adapter MVP shipped in the host adapter repo on branch `camouflage-adapter` (commit `b840b80`). Added `--emit-events` one-shot mode (`src/emit-mode.ts`) cloning the `runPrintMode` template; emits 8 event types (Session/UserMessage/AssistantStream/Tool/Permission/RuntimeError) via NDJSON to stdout. Smoke-tested with a real host turn — clean 6-event stream end-to-end. Multi-turn, bidirectional permission responses, `StatusUpdate` and `BackgroundTaskUpdate` deferred to follow-up commits. |
...
| 2026-05-17 | Ink-replacement-ready milestone shipped (tagged `v0.4.5`). Three-phase push: (1) host adapter completeness — 5 slices in the host adapter repo's `camouflage-adapter` branch (`8fc1093` → `a8c9108`): tool stdout/stderr emission, multi-turn mode, StatusUpdate (mode/phase/elapsed/tokens/branch), bidirectional permission flow, BackgroundTaskUpdate. (2) Camouflage TUI features — 4 slices on main (`2ced5a4` → `cbcd655`): expandable tool overlay (X), permission feedback input, slash-command picker (with new `SlashCommandsRegistered` event), `@`-mention picker (with new `MentionCandidatesRegistered` event). (3) TUI bug backlog cleared (`55e4958`): TB1/TB2/TB3 all fixed. +3 themes (nord/gruvbox-dark/tokyo-night). Protocol EventType variants 22 → 24. The host can now realistically swap Ink for Camouflage. |
| 2026-05-17 | Node SDK shipped — `camouflage` npm package (rename from `camouflage-sdk`). Five-commit series on main (`ebf748d` → `d826f89`) wraps the Rust renderer in a Node-native API: `mount() → send() / on() / close()`, hides all subprocess management and NDJSON plumbing. New `--responses-fd` flag on camouflage-tui (`97af9dd`) lets the renderer write outbound NDJSON to a separate fd so stdout can be reserved for rendering to the user's terminal. The host's `--ui camouflage` mode (`2f1c7de` on `camouflage-adapter`) is the first consumer: `your-agent --ui camouflage -p "..."` spawns the renderer in-process, user types follow-ups directly in the TUI, permissions round-trip without a single visible pipe. This is the integration shape that paves the way to deleting Ink entirely (Option B). |
...
| 2026-05-17 | CC-1 SelectList shipped end-to-end. 5 commits: protocol+payloads (`c7480b4`), model state + Snapshot (`ff6505e`), TUI overlay + key handling + outbound emit (`0e42e38`), Node SDK `selectList()` helper + `selectListResponse` event (`aac4094`), host `resume` subcommand as first real driver (`6556771` on `camouflage-adapter`). Protocol EventType variants 24 → 26. First worked example of the components-catalog migration playbook. |
| 2026-05-17 | CC-2 Confirm + CC-3 Toast shipped (`7bc8d43`, `8f3f0d4`). Protocol EventType variants 26 → 29. Both follow the same one-commit-per-component pattern: protocol+payloads → model+snapshot → TUI overlay → Node SDK helper. Three components live now (SelectList / Confirm / Toast); four left in the catalog (Wizard, Form, Table, KeyValueView). Toast is the first display-only primitive (no outbound response). Host drivers for CC-2/CC-3 are TODO. |
| 2026-05-17 | Components catalog COMPLETE — CC-4 Wizard + CC-5 Form + CC-6 Table + CC-7 KeyValueView shipped (`e4c1d25`, `348de66`, `801a8fd`, then Wizard final commit). Protocol EventType variants 29 → 36. All 7 catalog primitives now live: SelectList, Confirm, Toast, Wizard, Form, Table, KeyValueView. Wizard composes Select/Confirm/Form by intercepting sub-modal responses into a wizard-advance path that records per-step results and either installs the next step or emits WizardCompleted. Camouflage is now a complete TUI component library — hosts can describe any common UI shape declaratively. Host-side driver migrations (replacing src/ui/*.tsx files one by one with catalog-event emits) are the next phase. |
